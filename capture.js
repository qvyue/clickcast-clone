...
async function injectProbesAndExtractBlocks(page) {
  return await page.evaluate(() => {
    ...
    // 只关注这些容器元素
    const containerSelectors = ['section', 'article', 'main', 'header', 'footer'];
    ...
    containerSelectors.forEach(tag => {
      document.querySelectorAll(tag).forEach(el => containers.add(el));
    });

    // 2. 收集带语义化 class/id 的 div
    ...
    containers.forEach(el => {
      ...
      // 过滤掉导航栏
      if (className.includes('nav') || id.includes('nav')) return;
      ...
      // 判断区块类型（className 和 id 已在前面定义）
      const combined = `${className} ${id} ${textSummary}`.toLowerCase();
      ...
      else if (combined.includes('faq') || combined.includes('question')) suggestedType = 'faq';
      else if (combined.includes('cta') || combined.includes('contact')) suggestedType = 'cta';
      ...
      containerData.push({
        ...
        contentScore: innerText.length + (heading ? 100 : 0) + (ctaText ? 50 : 0)
      });
    });
...
TASK: Select the BEST sections for a promotional video. You decide the optimal number (3-8 sections).

SELECTION RULES:
1. QUALITY over QUANTITY: Only select sections with meaningful, unique content
2. If the page has limited content, select fewer sections (3-4 is fine)
3. If the page has rich content, you can select more (up to 8)
4. Prioritize: Hero section (only 1), Key features, Pricing, Testimonials, Call-to-action
5. DIVERSITY: Do NOT select multiple sections with similar content
6. Each selected section should show DIFFERENT aspects of the product
7. Skip sections with generic/placeholder content
...
Output JSON only:
{
  ...
  "reasons": {
    "id1": "Brief reason why this section was selected"
  }
}`;
  ...
  return new Promise((resolve) => {
        ...
        try {
          ...
          if (json.choices && json.choices[0]) {
            ...
            if (jsonMatch) {
              ...
              // 不再强制补齐，AI 已决定最佳数量
              console.log(`   🤖 AI 选择了 ${contentDeduped.length} 个区块`);
              resolve(contentDeduped);
            } else {
              // 解析失败，返回所有候选
              resolve(candidates);
            }
    ...
    req.on('error', () => resolve(candidates));
    req.write(postData);
    req.end();
  });
}
      ...
      document.querySelectorAll('h2, h3').forEach((h) => {
        ...
        if (absoluteTop > 400) {
          ...
          if (container) {
            ...
            if (!exists && containerRect.height > 150 && containerRect.height < 1200) {
...
async function checkScreenshotContentDensity(imagePath) {
  ...
  try {
    ...
    // 有意义的截图通常每像素 > 0.3 字节（压缩后）
    // 空白/单色图片通常 < 0.03 字节/像素
    // 阈值设置：太低会放过空白图，太高会拒绝有效截图
    const contentRatio = bytesPerPixel;

    if (pixelCount > 0 && bytesPerPixel < 0.03) {
      return {
        isValid: false,
        contentRatio,
...
async function capture() {
  ...
  const selectedBlocks = await aiSelectBlocks(candidates, pageContent);
  console.log(`   ✅ AI 选中 ${selectedBlocks.length} 个区块:`);
  ...
  for (let i = 0; i < selectedBlocks.length; i++) {
    ...
    try {
      ...
      await page.waitForTimeout(500); // 增加等待时间让内容加载
