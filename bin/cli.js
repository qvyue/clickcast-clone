...
async function captureWebsite(url, outputDir = './public') {
  ...
  return new Promise((resolve) => {
    const proc = spawn('node', ['../lib/capture.js', url, outputDir], {
      cwd: __dirname,
      stdio: 'inherit'
    });
    proc.on('close', (code) => {
...
async function main() {
  ...
  try {
    ...
    // 2.5 AI 智能分析图片裁切策略
    console.log('\n[2.5/5] 🖼️ AI 分析图片裁切策略...');
    const { analyzeImageCropStrategy } = require('../lib/ai-agent.js');
    ...
    // 4.5 AI 选择背景音乐
    console.log('\n[4.5/5] 🎵 AI 选择背景音乐...');
    const { selectBGMForVideo, generateBGMConfig } = require('../lib/bgm-selector.js');
