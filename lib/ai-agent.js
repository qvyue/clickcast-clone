...
2. Each scene should highlight ONE key benefit/feature
3. Order: Hook → Value Props → Social Proof → CTA
4. Use industry insights to make the script more relevant and competitive
5. IMPORTANT: If "WEBSITE OWNER'S KEY REQUIREMENTS" section exists above, prioritize those points!
...
6. CRITICAL: Match each scene with the MOST RELEVANT screenshot from the available screenshots list above.
...
=== LENGTH RULES (STRICT!) ===
- mainTitle: 3-8 words MAX. Count every word. If your title exceeds 8 words, rewrite it shorter.
- subTitle: 5-10 words MAX. Must add NEW info, not rephrase mainTitle.
- Aim for 10-15 total words per scene (mainTitle + subTitle combined).
- Prefer active verbs: "Automate", "Create", "Transform" over passive descriptions.
- Cut filler words: "the", "a", "your", "that", "which" — remove if meaning is preserved.
...
=== OUTPUT FORMAT ===
{
  ...
  "scenes": [
    {
      "id": "intro",
      "mainTitle": "Short punchy hook (3-8 words)",
      "subTitle": "",
    ...
    {
      "id": "scene0",
      "mainTitle": "Core benefit (3-8 words)",
      "subTitle": "Supporting detail (5-10 words)",
...
=== mainTitle vs subTitle RULES ===

RULE 1: subTitle is recommended but optional.
- If mainTitle already delivers a complete message, subTitle may be empty ("").
- intro and outro scenes often work best with mainTitle only.
...
EXAMPLES:
GOOD (short & punchy):
  mainTitle: "AI-powered scriptwriting"  (3 words)
  subTitle: "Generates voiceover scripts from any website"  (6 words)

GOOD (action-oriented):
  mainTitle: "One click to video"  (4 words)
  subTitle: "Paste any URL and get a complete marketing video"  (9 words)

BAD (too long — DO NOT write like this):
  mainTitle: "Our AI-powered platform automatically generates professional scripts from your website content"  (12 words)
  subTitle: "You can create amazing marketing videos without any editing experience required"  (11 words)
...
function validateVideoScriptLocal(script) {
  ...
  if (script.scenes) {
    script.scenes.forEach((scene, i) => {
      // mainTitle 必须有内容
      if (!scene.mainTitle || !scene.mainTitle.trim()) {
        issues.push(`Scene ${i} (${scene.id}): mainTitle is empty`);
        score -= 10;
      }
      // subTitle 为空是合法的（intro/outro 可以只有 mainTitle）
    });
  }
