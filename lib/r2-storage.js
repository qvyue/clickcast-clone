 ...
 * 用于宣传片尾等不按域名划分的共享文件
 * - 本地已存在 → 直接返回路径
 * - 不存在 + R2 已配置 → 从 R2 assets/ 前缀下载到本地
 * - 不存在 + R2 未配置 → 返回 null
 * @param {string} filename - 文件名 (如: promo-outro-landscape.mp4)
 * @param {string} publicDir - 本地 public 目录路径
 * @returns {Promise<string|null>} 本地文件路径，不可用时返回 null
 */
async function ensureSharedAsset(filename, publicDir) {
  const localPath = path.join(publicDir, filename);
  ...
  // 检查是否已有正在进行的下载
  if (inflightDownloads.has(r2Key)) {
    const success = await inflightDownloads.get(r2Key);
  ...
  const downloadPromise = (async () => {
    try {
      ...
      console.error(`R2 ensure shared asset error (${r2Key}):`, err.message);
      return false;
    } finally {
      inflightDownloads.delete(r2Key);
    }
...
/**
 * 删除 R2 上指定域名的所有资源文件
 * 列出 resources/{domain}/public/ 下的所有对象并逐个删除
 * @param {string} domain - 网站域名
 * @returns {Promise<{deleted: number, failed: number}>}
 */
async function deleteDomainResources(domain) {
  if (!isR2Configured()) return { deleted: 0, failed: 0 };

  const prefix = `resources/${domain}/public/`;
  const objects = await listResources(prefix);

  if (objects.length === 0) return { deleted: 0, failed: 0 };

  let deleted = 0, failed = 0;
  for (const obj of objects) {
    const result = await deleteVideo(obj.key);
    result ? deleted++ : failed++;
  }

  console.log(`   🗑️ R2 资源删除: ${domain} - ${deleted} 删除, ${failed} 失败`);
