...
describe('ElevenLabs 配置检查', () => {
  ...
  it('getAvailableVoices 应返回可用声音列表', () => {
    const expectedVoices = ['Adam', 'Rachel', 'Antoni', 'Josh', 'Bella', 'Dallin'];
    // 这些是硬编码的声音名称
    assert.strictEqual(expectedVoices.length, 6);
    assert.ok(expectedVoices.includes('Dallin'));
  });
});

describe('声音名称验证', () => {
  it('已知声音名称应被接受', () => {
    const validVoices = ['Adam', 'Rachel', 'Antoni', 'Josh', 'Bella', 'Dallin'];
    const voiceName = 'Dallin';

    const isValid = validVoices.includes(voiceName);
    assert.strictEqual(isValid, true);
  });

  it('未知声音名称应回退到默认声音', () => {
    const validVoices = ['Adam', 'Rachel', 'Antoni', 'Josh', 'Bella', 'Dallin'];
    const defaultVoice = 'Dallin';
    const unknownVoice = 'UnknownVoice';

    let selectedVoice = unknownVoice;
    if (!validVoices.includes(unknownVoice)) {
      selectedVoice = defaultVoice;
    }

    assert.strictEqual(selectedVoice, 'Dallin');
  });

  it('空声音名称应使用默认声音', () => {
    const defaultVoice = 'Dallin';
    const voiceName = null;
