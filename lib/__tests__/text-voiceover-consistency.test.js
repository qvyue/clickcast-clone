...
describe('enforceTimelineRules — 文案=配音一致性', () => {
  ...
  it('should ensure consistency for outro scene', () => {
    const timeline = {
      product: 'ClickCast',
      scenes: [
        { id: 'outro', mainTitle: 'Try ClickCast — Transform any website into videos.', subTitle: '' }
      ]
    };
    ...
    // mainTitle 不应该被截断
    assert.strictEqual(outro.mainTitle, 'Try ClickCast — Transform any website into videos.', 'outro: mainTitle should not be truncated');
  });
...
describe('AI Agent — 文案=配音一致性', () => {
  ...
  if (ensureFieldCompatibility) {
    ...
    it('ensureFieldCompatibility should fill default subTitle for short mainTitle', () => {
      ...
      // subTitle 应该是默认内容
      assert.ok(scene.subVoiceover.includes('ClickCast'),
...
describe('Generate flow — 文案=配音一致性', () => {
  ...
  it('timeline scene data should have consistent title/mainTitle and subVoiceover/subTitle', () => {
    const scenes = [
      { id: 'intro', mainTitle: 'Welcome to ClickCast.', subTitle: 'Discover the power of AI video.' },
      { id: 'scene0', mainTitle: 'AI-powered video generation.', subTitle: 'Transform any website into a video.' },
      { id: 'outro', mainTitle: 'Try ClickCast — stunning marketing videos.', subTitle: 'Get started today.' }
    ];
