...
const SELECTOR_CONFIG = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'cookie-selectors.json'), 'utf-8')
);

function buildCookieHideCSS() {
  const selectors = SELECTOR_CONFIG.hide_css_selectors || [];
  return selectors.join(',\n') + ' {\n  display: none !important;\n  visibility: hidden !important;\n  opacity: 0 !important;\n  pointer-events: none !important;\n  z-index: -9999 !important;\n}\nbody {\n  overflow: auto !important;\n  position: static !important;\n}';
}

async function setupCookieScriptBlocking(page) {
  const blockedDomains = SELECTOR_CONFIG.blocked_domains || [];
...
async function setupInitScriptHiding(context) {
  const css = buildCookieHideCSS();
...
function setupPopupGuard(context, mainPage) { 
  context.on('page', async (newPage) => {
    if (newPage === mainPage) return;
    if (!newPage.opener) return;
    try {
      const opener = await newPage.opener();
      if (!opener) return;
      console.log('   🛡️ 检测到新标签页弹出，自动关闭');
      await newPage.close();
    } catch (e) {}
  });
}
...
/**
 * 检测并关闭 Cookie/隐私弹窗
 * 改进：区分"安全接受按钮"和"危险导航按钮"，避免错误点击导致页面重定向
 */
async function dismissCookiePopup(page) {
  const selectors = SELECTOR_CONFIG.cookie_selectors || [];
    ...
    try {
      ...
      if (await button.isVisible({ timeout: 1000 }).catch(() => false)) {
        if (await button.isEnabled().catch(() => false)) {
          ...
          console.log(`   🍪 已关闭 Cookie 弹窗 (Locator: ${selector})`);

          await button.waitFor({ state: 'hidden', timeout: 2000 }).catch(() => {});
          await page.waitForTimeout(500);

          const urlAfterClick = page.url();
          if (urlAfterClick !== urlBeforeClick) {
            console.log(`   ⚠️ 点击后页面导航，正在返回原页面...`);
            await page.goto(urlBeforeClick, { waitUntil: 'domcontentloaded', timeout: 30000 });
  ...
  try {
    const acceptBtn = page.getByRole('button', { name: /accept all|agree|got it|okay|同意|接受|允许|确定/i }).first();
    if (await acceptBtn.isVisible({ timeout: 1000 }).catch(() => false)) {
      await acceptBtn.click({ force: true, timeout: 2000 }).catch(() => {});
      console.log(`   🍪 已关闭 Cookie 弹窗 (Role: button Accept/Agree)`);
      await page.waitForTimeout(500);
      return true;
    }
  } catch (e) {}
  ...
  try {
    ...
    const agreeBtn = dialog.locator("button:has-text('Accept'), button:has-text('Agree'), button:has-text('同意'), button:has-text('接受')").first();
    if (await agreeBtn.isVisible({ timeout: 1000 }).catch(() => false)) {
      await agreeBtn.click({ force: true, timeout: 2000 }).catch(() => {});
      console.log(`   🍪 已关闭 Cookie 弹窗 (Dialog 内 Accept/Agree 按钮)`);
      await page.waitForTimeout(500);
      return true;
    }
  } catch (e) {}
...
/**
 * 检查当前页面是否是 Cookie Policy 或 Privacy Policy 页面
 * @param {Page} page - Playwright 页面对象
 * @returns {Promise<boolean>} 是否在 Cookie/Privacy Policy 页面
 */
async function isOnCookiePolicyPage(page) {
  try {
    ...
    // 检查 URL
    if (url.includes('cookie-policy') ||
        url.includes('privacy-policy') ||
        url.includes('cookie-settings') ||
        url.includes('legal/cookies')) {
      return true;
    }

    // 检查页面标题
    if (lowerTitle.includes('cookie policy') ||
        lowerTitle.includes('privacy policy') ||
        lowerTitle.includes('cookie settings') ||
        lowerTitle.includes('cookie preferences')) {
      return true;
    }
...
/**
 * 确保当前页面在目标网站上，如果被重定向则返回原页面
 * @param {Page} page - Playwright 页面对象
 * @param {string} targetUrl - 目标 URL
 * @returns {Promise<boolean>} 是否发生了页面恢复
 */
async function ensureOnCorrectPage(page, targetUrl) {
  try {
    ...
    try {
      ...
      targetDomain = targetUrl.replace(/^https?:\/\//, '').split('/')[0];
    }

    // 检查是否在错误页面上
    const isWrongPage =
      !currentUrl.includes(targetDomain) ||
      currentUrl.toLowerCase().includes('cookie-policy') ||
      currentUrl.toLowerCase().includes('privacy-policy') ||
      await isOnCookiePolicyPage(page);

    if (isWrongPage) {
      console.log(`   ⚠️ 检测到页面已离开目标: ${currentUrl}`);
      console.log(`   🔙 正在返回目标页面: ${targetUrl}`);
      await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
      await page.waitForTimeout(2000);
      // 重新处理可能的 Cookie 弹窗
      await dismissCookiePopup(page);
      return true; // 表示发生了页面恢复
    }

    return false;
  } catch (e) {
    console.log(`   ⚠️ ensureOnCorrectPage 错误: ${e.message}`);
    return false;
  }
}
...
async function dismissModalDialogs(page) {
  ...
  const closeSelectors = SELECTOR_CONFIG.close_selectors || [];
  ...
  for (const selector of closeSelectors) {
    try {
      ...
      if (await button.isVisible({ timeout: 500 }).catch(() => false)) {
        await button.click({ force: true, timeout: 2000 }).catch(() => {});
        console.log(`   ✖️ 已点击关闭按钮 (Locator: ${selector})`);
        await button.waitFor({ state: 'hidden', timeout: 2000 }).catch(() => {});
 ...
 */
async function checkForVisiblePopups(page) {
  return await page.evaluate(() => {
    const selectors = [
      '[role="dialog"]',
      '[class*="modal"]',
...
async function removeVisiblePopups(page) {
  await page.evaluate(() => {
    const selectors = [
      '[role="dialog"]',
...
async function ensureCleanPage(page, maxAttempts = 2) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const hasPopup = await checkForVisiblePopups(page);
    if (!hasPopup) {
...
/**
 * 截图前彻底中和 web font，避免 Playwright screenshot 内部
 * "waiting for fonts to load" 在网络不通时卡死。
 *
 * 做两件事：
 * 1. 清除所有 stylesheet 中的 @font-face 规则
 * 2. 清空 document.fonts 集合 → fonts.ready 立即 resolve
 *
 * 副作用：截图中将看到 fallback 字体（如 system-ui），但能保证截图成功
 */
async function neutralizeFonts(page) {
  try {
    await page.evaluate(() => {
...
async function capture() {
  ...
  await setupInitScriptHiding(context);

  const page = await context.newPage();

  setupPopupGuard(context, page);

  await setupCookieScriptBlocking(page);

  let gotoSuccess = false;
  const maxRetries = 3;
  for (let retry = 0; retry < maxRetries && !gotoSuccess; retry++) {
    try {
      if (retry > 0) {
        console.log(`   🔄 重试访问 (${retry}/${maxRetries})...`);
        await page.waitForTimeout(2000);
      }
  ...
  try {
    ...
    console.log('   ⚠️ 等待字体超时，继续执行');
  }

  try {
    await page.addStyleTag({
      content: `* { font-display: swap !important; }`
    });
    console.log('   🔤 已注入 font-display:swap');
  } catch (e) {
    console.log('   ⚠️ 字体优化注入失败，继续执行');
  }

  await neutralizeFonts(page);

  const originalUrl = page.url();
  console.log(`   📍 目标 URL: ${originalUrl}`);
  ...
  await page.waitForTimeout(1000);
  const currentUrl = page.url();
  if (currentUrl !== originalUrl && !currentUrl.startsWith(originalUrl.split('?')[0])) {
    console.log(`   ⚠️ 检测到页面重定向: ${originalUrl} -> ${currentUrl}`);
    console.log('   🔙 正在返回原页面...');
    try {
      ...
      console.log(`   ⚠️ 返回原页面失败: ${e.message}`);
    }
  }
  ...
  // 检测是否在 Cookie Policy / Privacy Policy 页面
  const pageTitle = (pageContent.seo?.title || '').toLowerCase();
  const pageUrl = pageContent.url || '';
  const isPolicyPage = pageTitle.includes('cookie policy') ||
                       pageTitle.includes('privacy policy') ||
                       pageTitle.includes('cookie-policy') ||
                       pageTitle.includes('privacy-policy') ||
                       pageUrl.includes('/cookie') && pageUrl.includes('policy') ||
                       pageUrl.includes('/privacy') && pageUrl.includes('policy');

  if (isPolicyPage) {
    console.log(`   ⚠️ 检测到 Cookie/Privacy Policy 页面: "${pageTitle}"`);
    console.log('   🔙 正在返回原页面...');
  ...
  for (let i = 0; i < sortedCandidates.length; i++) {
    ...
    try {
      ...
      await element.screenshot({ path: debugPath, timeout: 10000, animations: 'disabled' });
  ...
  for (let i = 0; i < selectedBlocks.length; i++) {
    ...
    // 在每次截图前验证当前页面是否在正确位置
    const wasRedirected = await ensureOnCorrectPage(page, url);
    if (wasRedirected) {
      console.log(`   🔄 页面已恢复，重新获取元素...`);
    }
    ...
    try {
      ...
      await removeVisiblePopups(page);
      await page.waitForTimeout(200);

      // 先截图到临时路径
      const tempPath = path.join(publicDir, `_temp_shot.png`);
      await element.screenshot({ path: tempPath, timeout: 10000, animations: 'disabled' });
  ...
  let minScreenshots;
  if (maxScroll < 200) {
    minScreenshots = 3;  // 极短页面至少 3 张
  } else {
    // 根据页面高度计算：每 600px 滚动一张截图，最少 5 张，最多 8 张
    const scrollPages = Math.ceil(maxScroll / 600);
    minScreenshots = Math.max(5, Math.min(scrollPages + 2, 8));
  }

  // 如果 AI 区块提取失败（没有选中任何区块），增加截图数量
  const needMoreScreenshots = selectedBlocks.length === 0;
  if (needMoreScreenshots) {
    console.log('   ⚠️ AI 区块提取失败，使用增强滚动截图模式');
    minScreenshots = Math.max(minScreenshots, 6);  // 至少 6 张
  }

  console.log(`   📊 最小截图数量: ${minScreenshots} (当前: ${screenshots.length})`);
  ...
  if (screenshots.length < minScreenshots) {
    ...
    for (let i = 0; i < scrollPositions.length && screenshots.length < minScreenshots; i++) {
      ...
      const wasRedirected = await ensureOnCorrectPage(page, url);
      if (wasRedirected) {
        console.log(`   🔄 页面已恢复，重新滚动到位置 ${scrollY}px...`);
        await page.evaluate((y) => window.scrollTo(0, y), scrollY);
        await page.waitForTimeout(1000);
      }
      ...
      const shotIndex = screenshots.length + 1;
      const tempPath = path.join(publicDir, `_temp_shot.png`);
      await page.screenshot({ path: tempPath, fullPage: false, timeout: 10000, animations: 'disabled' });
