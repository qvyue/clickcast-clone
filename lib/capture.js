...
/**
 * Find available Chromium executable for Playwright.
 * Tries Remotion's browser first, then Playwright's default.
 */
function findChromiumPath() {
  ...
  // 2. Remotion's browser cache (~/.cache/remotion/chromium/)
  const remotionCache = path.join(os.homedir(), '.cache', 'remotion', 'chromium');
  if (fs.existsSync(remotionCache)) {
    try {
      const dirs = fs.readdirSync(remotionCache).filter(d => {
        const candidate = process.platform === 'linux'
          ? path.join(remotionCache, d, 'chrome-linux64', 'chrome')
          : process.platform === 'darwin'
            ? path.join(remotionCache, d, 'chrome-mac', 'Chromium.app', 'Contents', 'MacOS', 'Chromium')
            : path.join(remotionCache, d, 'chrome-win64', 'chrome.exe');
        return fs.existsSync(candidate);
      });
      if (dirs.length > 0) {
        const candidate = process.platform === 'linux'
          ? path.join(remotionCache, dirs[0], 'chrome-linux64', 'chrome')
          : process.platform === 'darwin'
            ? path.join(remotionCache, dirs[0], 'chrome-mac', 'Chromium.app', 'Contents', 'MacOS', 'Chromium')
            : path.join(remotionCache, dirs[0], 'chrome-win64', 'chrome.exe');
        if (fs.existsSync(candidate)) {
          console.log(`   🌐 Using Remotion browser: ${candidate}`);
          return candidate;
        }
      }
    } catch (e) { /* ignore */ }
  ...
  // 3. Fallback to Playwright's default (let Playwright find its own browser)
  return undefined;
}
...
async function dismissCookiePopup(page) {
  // 安全的接受按钮文本（优先点击）
  const safeAcceptPatterns = [
    'accept all', 'accept all cookies', 'got it', 'okay', 'ok',
    'i agree', '同意', '接受', '允许', '确定', 'allow all', 'agree to all'
  ];

  // 可能导航的按钮文本（需要谨慎）
  const cautiousAcceptPatterns = [
    'accept', 'agree', 'continue', 'allow'
  ];

  // 危险的导航按钮文本（避免点击）
  const dangerousPatterns = [
    'settings', 'manage', 'customize', 'learn more', 'options',
    'preferences', 'details', 'cookie policy', 'read more', 'change',
    'configure', 'edit', 'view', 'more information'
  ];
  ...
  const safeSelectors = [
    ...
    'button.cc-accept', 'button.cc-dismiss',
    '[data-testid="cookie-banner-accept"]',
    '[data-testid*="accept-all"]',
  ];

  // 检查按钮文本是否包含危险关键词
  const isDangerousButton = (text) => {
    const lowerText = text.toLowerCase();
    return dangerousPatterns.some(pattern => lowerText.includes(pattern));
  };

  // 检查按钮是否是安全的接受按钮
  const isSafeAcceptButton = (text) => {
    const lowerText = text.toLowerCase();
    return safeAcceptPatterns.some(pattern => lowerText.includes(pattern));
  };
      ...
      if (element) {
        ...
        if (isVisible) {
          ...
          if (!isDangerousButton(text)) {
            ...
            console.log(`   🍪 已关闭 Cookie 弹窗 (安全选择器: ${selector})`);
  ...
  for (const el of allButtons) {
    try {
      ...
      // 分类按钮
      if (isSafeAcceptButton(text)) {
        safeButtons.push({ el, text: text.substring(0, 50) });
      } else if (cautiousAcceptPatterns.some(pattern => text.toLowerCase().includes(pattern))) {
        cautiousButtons.push({ el, text: text.substring(0, 50) });
      }
    } catch (e) {}
  }
  ...
  // 优先点击安全按钮
  for (const { el, text } of safeButtons) {
    try {
      await el.click({ timeout: 2000 }).catch(() => {});
      console.log(`   🍪 已关闭 Cookie 弹窗 (安全按钮: "${text.substring(0, 30)}")`);
      await page.waitForTimeout(800);

      // 检查是否发生了页面导航
      const urlAfterClick = page.url();
      if (urlAfterClick !== urlBeforeClick) {
        console.log(`   ⚠️ 点击按钮后页面导航: ${urlBeforeClick} -> ${urlAfterClick}`);
        console.log(`   🔙 正在返回原页面...`);
  ...
  // 其次点击谨慎按钮
  for (const { el, text } of cautiousButtons) {
    try {
      await el.click({ timeout: 2000 }).catch(() => {});
      console.log(`   🍪 已关闭 Cookie 弹窗 (谨慎按钮: "${text.substring(0, 30)}")`);
      await page.waitForTimeout(800);
  ...
  for (let retry = 0; retry < maxRetries && !gotoSuccess; retry++) {
    try {
      if (retry > 0) {
        ...
        await page.waitForTimeout(2000); // 重试前等待
      }
